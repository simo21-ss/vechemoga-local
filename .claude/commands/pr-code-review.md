...
- **A single `.mjs` bind-mounted into a stock image** rather than a built image — intentional for the stack; there is nothing to install. `provider-proxy/Dockerfile` exists only for running the proxy where this repo isn't checked out, and the compose deliberately does not use it.
...
- **No *app* Dockerfiles here** — those live in the app repos; this repo only composes them. (`provider-proxy/` is this repo's own source, so its Dockerfile is not an exception to that rule.)
