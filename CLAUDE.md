...
**The API's email is turned on *here*, not in its profile.** The Spring `local` profile leaves transactional
email off (a bare host run has nowhere to send it) — but this stack *does* run a capture server, so the compose
enables it via `JAVA_OPTS` `-D` properties (`vechemoga.email.enabled=true`,
`vechemoga.email.loops.base-url=http://provider-proxy:1080`). Use the **`-D` form, not env vars**: relaxed
binding mangles `base-url` into `..._BASEURL`, which is easy to get wrong silently.
...
(Node built-ins only) for the API's outbound HTTP providers, of which mail is today's only one. It runs as the
`provider-proxy` service and is the API's provider base URL for the whole stack, tests or not. A client
registers an expectation over the HTTP control plane (`/__proxy/*`) and reads the captured send back out;
unmatched traffic goes to `PROXY_UPSTREAM_URL`. Its contract is documented in
[provider-proxy/README.md](provider-proxy/README.md) and pinned by `node --test test/`.

- **`PROXY_UPSTREAM_URL` is unset by default, deliberately.** Unmatched traffic is a test that forgot to
  register an expectation, or a hand-registration in the browser — neither should reach a real inbox, so both
  502 loudly. `.env.example` documents `https://app.loops.so/api` as the opt-in. The API's empty
  `LOOPS_API_KEY` is **not** a safety net; it only makes a leaked send 401.
...
- Mounted **read-only into stock `node:20-alpine`**: the script needs no install, so there is no image to build.
